// AUTO-GENERATED by vue/scripts/generate-vue-icons.ts — do not edit manually.
// Source: react/src/icons/components/Host.tsx
import { defineComponent, h } from "vue";

export const Host = defineComponent({
  name: "Host",
  setup() {
    return () =>
      h("svg", {
        "width": "16px",
        "height": "16px",
        "viewBox": "0 0 16 16",
        "version": "1.1",
        "xmlns": "http://www.w3.org/2000/svg",
        innerHTML: "<title>catalog_provider</title><g id=\"catalog_provider\" stroke=\"none\" stroke-width=\"1\" fill=\"none\" fill-rule=\"evenodd\" ><path d=\"M5,2 C5.55228475,2 6,2.44771525 6,3 L6,14 C6,14.5522847 5.55228475,15 5,15 L3,15 C2.44771525,15 2,14.5522847 2,14 L2,3 C2,2.44771525 2.44771525,2 3,2 L5,2 Z M11,2 C11.5522847,2 12,2.44771525 12,3 L12,14 C12,14.5522847 11.5522847,15 11,15 L9,15 C8.44771525,15 8,14.5522847 8,14 L8,3 C8,2.44771525 8.44771525,2 9,2 L11,2 Z M4,11 C3.44771525,11 3,11.4477153 3,12 C3,12.5522847 3.44771525,13 4,13 C4.55228475,13 5,12.5522847 5,12 C5,11.4477153 4.55228475,11 4,11 Z M10,11 C9.44771525,11 9,11.4477153 9,12 C9,12.5522847 9.44771525,13 10,13 C10.5522847,13 11,12.5522847 11,12 C11,11.4477153 10.5522847,11 10,11 Z\" id=\"Combined-Shape\" fill=\"currentColor\" ></path></g>",
      });
  },
});
