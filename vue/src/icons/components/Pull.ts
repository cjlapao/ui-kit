// AUTO-GENERATED by vue/scripts/generate-vue-icons.ts — do not edit manually.
// Source: react/src/icons/components/Pull.tsx
import { defineComponent, h } from "vue";

export const Pull = defineComponent({
  name: "Pull",
  setup() {
    return () =>
      h("svg", {
        "width": "16px",
        "height": "16px",
        "viewBox": "0 0 16 16",
        "version": "1.1",
        "xmlns": "http://www.w3.org/2000/svg",
        innerHTML: "<path d=\"M4,10 L4,10.999 L3,11 C2.44771525,11 2,11.4477153 2,12 L2,13 C2,13.5522847 2.44771525,14 3,14 L12,14 C12.5522847,14 13,13.5522847 13,13 L13,12 C13,11.4477153 12.5522847,11 12,11 L11,11 L11,10 L12,10 C13.1045695,10 14,10.8954305 14,12 L14,13 C14,14.1045695 13.1045695,15 12,15 L3,15 C1.8954305,15 1,14.1045695 1,13 L1,12 C1,10.8954305 1.8954305,10 3,10 L4,10 Z M8,1.93553715 L7.99920454,9.11 L9.64495201,7.46446609 L10.3520588,8.17157288 L7.52363166,11 L4.69520454,8.17157288 L5.40231132,7.46446609 L6.99920454,9.062 L7,1.93553715 L8,1.93553715 Z\" fill=\"currentColor\" fill-rule=\"nonzero\" ></path>",
      });
  },
});
