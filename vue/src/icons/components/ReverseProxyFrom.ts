// AUTO-GENERATED by vue/scripts/generate-vue-icons.ts — do not edit manually.
// Source: react/src/icons/components/ReverseProxyFrom.tsx
import { defineComponent, h } from "vue";

export const ReverseProxyFrom = defineComponent({
  name: "ReverseProxyFrom",
  setup() {
    return () =>
      h("svg", {
        "width": "16px",
        "height": "16px",
        "viewBox": "0 0 16 16",
        "version": "1.1",
        "xmlns": "http://www.w3.org/2000/svg",
        innerHTML: "<path d=\"M10.227436,5.63934145 L13.0558631,8.46776857 L10.227436,11.2961957 L9.52032918,10.5890889 L11.1654002,8.94334145 L5.99140023,8.94413691 L5.99181061,8.65768667 C5.91300669,9.41199683 5.2751711,10 4.5,10 C3.67157288,10 3,9.32842712 3,8.5 C3,7.67157288 3.67157288,7 4.5,7 C5.27520319,7 5.9130595,7.58805186 5.9918204,8.34240702 L5.99140023,7.94413691 L11.1174002,7.94334145 L9.52032918,6.34644823 L10.227436,5.63934145 Z M4.5,8 C4.22385763,8 4,8.22385763 4,8.5 C4,8.77614237 4.22385763,9 4.5,9 C4.77614237,9 5,8.77614237 5,8.5 C5,8.22385763 4.77614237,8 4.5,8 Z\" fill=\"currentColor\" ></path>",
      });
  },
});
