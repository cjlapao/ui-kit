// AUTO-GENERATED by vue/scripts/generate-vue-icons.ts — do not edit manually.
// Source: react/src/icons/components/ViewRows.tsx
import { defineComponent, h } from "vue";

export const ViewRows = defineComponent({
  name: "ViewRows",
  setup() {
    return () =>
      h("svg", {
        "viewBox": "0 0 24 24",
        "fill": "none",
        "xmlns": "http://www.w3.org/2000/svg",
        innerHTML: "<path d=\"M5.853 10.903c-.626 0-1.092-.15-1.4-.453C4.151 10.143 4 9.668 4 9.027v-3.15c0-.636.151-1.108.453-1.416.308-.307.774-.461 1.4-.461h11.294c.62 0 1.082.154 1.384.461.307.308.461.78.461 1.416v3.15c0 .641-.154 1.116-.461 1.423-.302.302-.764.454-1.384.454H5.853zm.008-1.447h11.27c.138 0 .241-.034.31-.103.07-.07.104-.178.104-.326V5.869c0-.143-.035-.25-.104-.318-.069-.069-.172-.103-.31-.103H5.861c-.138 0-.241.034-.31.103-.069.069-.103.175-.103.318v3.158c0 .148.034.257.103.326.069.069.172.103.31.103zM5.853 19c-.626 0-1.092-.154-1.4-.461-.302-.308-.453-.78-.453-1.416v-3.15c0-.641.151-1.113.453-1.415.308-.308.774-.461 1.4-.461h11.294c.62 0 1.082.153 1.384.46.307.303.461.775.461 1.416v3.15c0 .636-.154 1.108-.461 1.416-.302.307-.764.461-1.384.461H5.853zm.008-1.448h11.27c.138 0 .241-.034.31-.103.07-.069.104-.175.104-.318v-3.165c0-.144-.035-.25-.104-.319-.069-.069-.172-.103-.31-.103H5.861c-.138 0-.241.034-.31.103-.069.07-.103.175-.103.319v3.165c0 .143.034.25.103.318.069.069.172.103.31.103z\" fill=\"currentColor\" />",
      });
  },
});
