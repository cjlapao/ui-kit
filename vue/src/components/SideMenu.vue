...
type SideMenuRenderEntry =
  ...
  | { kind: "group"; key: string; item: SideMenuItemGroup; first: boolean }
  | {
      kind: "link";
      key: string;
      item: SideMenuItemLink;
      active: boolean;
      tokens: SideMenuColorTokens;
    };
</script>
...
const visibleItems = computed(() => {
  const { items, guardEvaluator, activeModuleView, moduleViewOptions } = props;
...
const renderEntries = computed<SideMenuRenderEntry[]>(() =>
  visibleItems.value.map((item, index) => {
    if (item.type === "divider") {
      return { kind: "divider", key: `divider-${index}` };
    }
    if (item.type === "group") {
      return {
        kind: "group",
    ...
    return {
      ...
      key: linkItem.path,
      item: linkItem,
      active: isActive(linkItem.path),
      tokens: getSideMenuColorTokens(linkItem.color || props.color),
    };
  }),
);
...
const desktopClasses = computed(
  () => `
    hidden md:flex flex-col flex-shrink-0 relative ${props.fullHeight ? "h-full" : "sticky top-16 h-[calc(100vh-64px)]"}
    transition-all duration-300
    shadow-[10px_0_30px_-10px_rgba(0,0,0,0.1)] dark:shadow-[10px_0_30px_-10px_rgba(0,0,0,0.4)] overflow-hidden
    ${isCollapsed.value ? "w-[68px]" : "w-64"}
  `,
);
...
<template>
  ...
  <div
    v-if="mobileOpen"
    class="fixed inset-0 z-50 bg-black/20 backdrop-blur-sm md:hidden"
    @click="onCloseMobile?.()"
  />

  <!-- Mobile Sidebar (isMobile = true) / Desktop Sidebar (isMobile = false) -->
  <aside
    v-for="isMobile in [true, false]"
    :key="isMobile ? 'mobile' : 'desktop'"
    :class="classNames(isMobile ? mobileClasses : desktopClasses, classAttr)"
  >
    ...
    <div
      v-if="!isMobile"
      class="absolute inset-0 backdrop-blur-2xl bg-white/70 dark:bg-neutral-900/90 pointer-events-none"
    />
    <!-- Dither Noise Overlay -->
    <div
      class="absolute inset-0 opacity-[0.4] pointer-events-none mix-blend-overlay"
      :style="noiseStyle"
    />
    <div class="relative h-full flex flex-col w-full">
      ...
      >
        ...
        <div
          v-if="logoText || $slots.logoText"
          :class="`overflow-hidden transition-all duration-300 ${isCollapsed ? 'w-0 opacity-0 ml-0' : 'w-auto opacity-100 ml-3'}`"
        >
          <div class="whitespace-nowrap">
            <slot name="logoText"><VNodeRenderer :nodes="logoText" /></slot>
          </div>
        </div>
      </div>
      ...
      <div
        v-if="title || isMobile"
        :class="`px-6 pt-4 pb-2 flex items-center ${isCollapsed && !isMobile ? 'justify-center px-3' : 'justify-between'}`"
      >
        <h2
          v-if="title && !(isCollapsed && !isMobile)"
          class="text-xs font-semibold text-gray-400 dark:text-neutral-500 uppercase tracking-wider whitespace-nowrap"
        >
          {{ title }}
        </h2>
        <!-- Mobile Close Button -->
        <button
          v-if="isMobile"
          class="p-1 rounded-lg hover:bg-white/50 dark:hover:bg-neutral-700/50 text-gray-400 dark:text-neutral-500 hover:text-gray-700 dark:hover:text-neutral-200 transition-colors ml-auto"
          @click="onCloseMobile?.()"
        >
          <CustomIcon icon="Close" class="w-5 h-5" />
        </button>
      </div>

      <!-- Navigation Items -->
      <div
        class="flex-1 px-3 py-1 overflow-y-auto w-full [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-neutral-200 dark:[&::-webkit-scrollbar-thumb]:bg-neutral-700 hover:[&::-webkit-scrollbar-thumb]:bg-neutral-300 dark:hover:[&::-webkit-scrollbar-thumb]:bg-neutral-600 [&::-webkit-scrollbar-track]:bg-transparent"
      >
        <nav class="space-y-1 w-full">
          <template v-for="entry in renderEntries" :key="entry.key">
            ...
            <div
              ...
              :class="`my-2 border-t border-gray-200/60 dark:border-neutral-700/60 ${isCollapsed && !isMobile ? 'mx-1' : 'mx-0'}`"
            />
            ...
            <template v-else-if="entry.kind === 'group'">
              <template v-if="!(isCollapsed && !isMobile)">
                <div
                  v-if="entry.item.hasDivider"
                  :class="`my-2 border-t border-gray-200/60 ${isCollapsed && !isMobile ? 'mx-1' : 'mx-0'}`"
                />
                <div
                  :class="`px-3 py-1 mb-1 text-xs font-semibold text-gray-400 dark:text-neutral-500 uppercase tracking-wider ${entry.first ? 'mt-1' : 'mt-4'}`"
                >
                  {{ entry.item.label }}
                </div>
              </template>
            ...
            <RouterLink
              v-else
              :to="entry.item.path"
              :title="isCollapsed && !isMobile ? entry.item.label : undefined"
              :class="`relative group flex items-center px-3 py-2.5 text-sm font-medium rounded-lg transition-all duration-150 ${
                entry.active
                  ? `${entry.tokens.bg} ${entry.tokens.text} shadow-sm`
                  : `text-gray-600 dark:text-neutral-400 ${entry.tokens.hoverBg} ${entry.tokens.hoverText}`
              } ${isCollapsed && !isMobile ? 'justify-center' : ''}`"
              @click="handleLinkClick"
            >
              <div
                v-if="entry.item.icon"
                :class="`flex items-center justify-center relative shrink-0 ${isCollapsed && !isMobile ? '' : 'mr-3'}`"
              >
                <CustomIcon
                  :icon="entry.item.icon"
                  :class="`h-5 w-5 transition-colors duration-150 ${entry.active ? entry.tokens.iconActive : `text-gray-400 dark:text-neutral-500 ${entry.tokens.iconHover}`}`"
                />
                <!-- Badge in collapsed mode: small dot over the icon -->
                <span
                  v-if="isCollapsed && !isMobile && entry.item.badge"
                  class="absolute -top-1 -right-1"
                >
                  <VNodeRenderer :nodes="entry.item.badge" />
                </span>
              </div>
              <span
      ...
      <div
        v-if="!isMobile"
        class="shrink-0 border-t border-gray-200/60 dark:border-neutral-700/60 px-3 py-3"
      >
        <button
        ...
        >
          ...
          />
          <span v-if="!isCollapsed" class="ml-3 whitespace-nowrap">Collapse</span>
        </button>
      </div>
    </div>
  </aside>
