...
├─ layout.ts             buildRows, rollupProgress, applyRowReorder, resolveDropBeforeId, reorderPreviewTop, reorderDragSubtree
...
| Progress edit | `progressFromPointer` | knob drag → `progress`. Knobs exist on **leaf tasks only** — a task with children displays its children's roll-up instead (read-only; its own `progress` is ignored). **Live preview while dragging** (both kits): the pointer maps to 0..1 on every move (`liveProgress` on the drag state), so the bar's progress fill, the knob, a floating `%` chip above the knob, the Progress-column cell, **the parent group's roll-up** and **the owning lane header's roll-up** all follow in real time — the fill/label/`title`/`aria-label` update mid-drag, and only the drop commits (`onTasksChange` / `tasks-change`). The un-done part of a bar is a **light tint** (`tokens.base`: `bg-{c}-300/70`, hover step `bg-{c}-400/70`; dark `dark:bg-{c}-400/35`), the done part is the darker `bg-{c}-700/70` progress overlay — done vs. remaining reads at a glance; the bar label switches dark/white text at 50% so it stays legible on both. Chips/owner initials keep the saturated `tokens.fill` |
| Row reorder (differentiator) | `applyRowReorder` + `resolveDropBeforeId` (block midpoint) + `reorderPreviewTop` + `reorderDragSubtree` | top-level **and** child (sibling) reorders. **SmartGrid-style drag system** (both kits): on grip press the object lifts into a **floating clone** (a pointer-event drag has no browser drag image — the clone follows the pointer at the grab offset, with an accent ring + the accent-coloured grip, and an opaque surface); a group drag carries its **whole expanded block** (group row + visible children, via `reorderDragSubtree`) and its in-flow slot becomes a **dashed ghost placeholder spanning the block's full height**, while the other rows close up. The drop target is decided at the **hovered block's midpoint** (same before/after rule as SmartGridLayout): a block = a row and its visible descendants (top-level drags see other top-level subtrees of the lane; child drags see their siblings). Above the midpoint → before the block; below → after it; over the object being dragged → the current target is kept, so a fresh press and small jitters shift nothing and the fixed-point (pointer lands over the moved object) keeps the preview from oscillating. Moving a row *down one* happens once the pointer has crossed the first row's midpoint — not "almost over the second item". Once decided, the live order re-derives (`applyRowReorder`) so the ghost slides into the prospective slot with the rows shifting in real time, the accent insertion line tracks the slot's edge (`reorderPreviewTop` on the live order, including the block's bottom edge for an end drop), and clone + line + ghost clear on drop, which commits the new order — top-level via `onReorder` / `reorder`, child-level via `onTasksChange` / `tasks-change` (sibling order is the tasks-array order). Dropping without ever deciding a target leaves the order unchanged; a child never crosses into another group (v1) |
...
| Hierarchy + roll-up progress | `buildRows` + `rollupProgress` + `taskRollupProgress` + `laneRollupProgress` | duration-weighted. **Groups have no progress of their own** — a group row/bar displays the roll-up of its children (read-only, no progress knob) and re-rolls live while one of its descendants is edited (`taskRollupProgress(tasks, taskId, override)` — a move/resize changes the duration weights, the knob changes the leaf value). The owning lane header's Progress cell re-rolls the same way; committed only on drop |
...
- **Safelist:** the dynamic classes the tokens emit are added to `scripts/generate-safelist.mjs`
  (a Gantt section: `bg-{c}-700/70`, `bg-{c}-300/70`, `dark:bg-{c}-400/35`,
  `hover:bg-{c}-400/70`, `dark:hover:bg-{c}-400/55`, `dark:bg-{c}-950`,
  `dark:bg-{c}-950/40`,
