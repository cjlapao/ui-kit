...
├─ layout.ts             buildRows, rollupProgress, applyRowReorder, resolveDropBeforeId, reorderPreviewTop
...
| Row reorder (differentiator) | `applyRowReorder` + `resolveDropBeforeId` (25% hysteresis) + `reorderPreviewTop` | within-lane in v1. **SmartGrid-style drag system** (both kits): on grip press the row lifts into a **floating clone** (a pointer-event drag has no browser drag image — the clone follows the pointer at the grab offset, with an accent ring + the accent-coloured grip, and an opaque surface) while its in-flow slot becomes a **dashed ghost placeholder** and the other rows close up. The drop target only commits once the pointer is in the **middle 50% of a candidate row** — the top/bottom 25% edge zones keep the previous target (hysteresis), so a fresh press and small jitters shift nothing and the preview never flickers at row boundaries. Once committed, the live order re-derives (`applyRowReorder`) so the ghost slides into the prospective slot with the rows shifting in real time, the accent insertion line tracks the ghost's top edge (`reorderPreviewTop` on the live order), and the floating clone + line + ghost all clear on drop, which commits the new `rowOrder` (`onReorder` / `reorder`). Dropping without ever committing leaves the order unchanged |
