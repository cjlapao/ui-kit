...
| Progress edit | `progressFromPointer` | knob drag → `progress`. **Live preview while dragging** (both kits): the pointer maps to 0..1 on every move (`liveProgress` on the drag state), so the bar's progress fill, the knob, a floating `%` chip above the knob, the Progress-column cell **and the owning lane header's roll-up** all follow in real time — the fill/label/`title`/`aria-label` update mid-drag, and only the drop commits (`onTasksChange` / `tasks-change`). The un-done part of a bar is a **light tint** (`tokens.base`: `bg-{c}-300/70`, hover step `bg-{c}-400/70`; dark `dark:bg-{c}-400/35`), the done part is the darker `bg-{c}-700/70` progress overlay — done vs. remaining reads at a glance; the bar label switches dark/white text at 50% so it stays legible on both. Chips/owner initials keep the saturated `tokens.fill` |
...
| Hierarchy + roll-up progress | `buildRows` + `rollupProgress` + `laneRollupProgress` | duration-weighted. **Live re-roll:** while any child is dragged, the owning lane header's Progress cell recomputes in real time from the previewed edit (`laneRollupProgress(tasks, taskId, override)` — a move/resize changes the duration weights, the knob changes the leaf value); committed only on drop |
