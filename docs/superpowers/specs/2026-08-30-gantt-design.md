...
| Row reorder (differentiator) | `applyRowReorder` + `resolveDropBeforeId` + `reorderPreviewTop` | within-lane in v1. **Live preview while dragging** (both kits): the pointer resolves a `beforeId` against the *currently rendered* rows (a stable fixed point — no oscillation), the row order re-derives on every move, so the dragged row (with its bar, cells and links) physically moves into its prospective slot while the other rows shift, and the accent insertion line tracks the edge of that slot (`reorderPreviewTop` on the live order). The dragged row is highlighted with the accent colour (grip icon in the accent hue + a subtle accent edge/tint on the row). Only the drop commits the new `rowOrder` (`onReorder` / `reorder`) |
