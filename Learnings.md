...
| 2026-07-10 | TypeScript | When using `Record<UnionType, V>` where the lookup object defines only a subset of union members, use `Extract<UnionType, SubUnion>` for the Record key type; assigning a partial object to `Record<FullUnion, V>` fails TS exhaustive-keys check. | toggle-type-unification T3 |
| 2026-07-10 | TypeScript | When building lookup tables keyed by a type union member subset, narrow the Record keys to literals (e.g., `Record<"sm"|"md"|"lg", ...>`) and guard indexing with the `in` operator — never index a Record with a value typed as the full union without checking existence first. | toggle-type-unification T2 |
| 2026-07-10 | Type Migration | When replacing a local type with a shared theme type, verify downstream consumers that pass the old type to unrelated types (e.g., PanelPadding) — compatible string literals can flow through without error even though the types serve different domains, so audit the full call chain, not just the direct import site. | toggle-type-unification T4 |